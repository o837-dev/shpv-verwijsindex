        ...
        public static LinkRegistrationResponse VerifyLink(LinkRegistrationRequest req)
        {
            ...
            if (provs.Count > 0) //if (VerifyProvider(req))
            {
                ...
                else if (ls.Count == 1)
                {
                    ...
                    /*
                    LINK ------[====]------ Bestaande link
                    REQ  ---------[===]---- Begin van request overlapt bestaande Link, -> check! [A]

                    LINK ------[====]------ Bestaande link
                    REQ  ----[===]--------- Eind van Request overlapt bestaande Link, -> check! [B]

                    LINK ------[====]------ Bestaande link
                    REQ  -[=============]-- Begint voor en eindigt na bestaande Link, -> check! [C]

                    LINK ------[====]------ Bestaande link
                    REQ  -------[==]------- Valt binnen een bestaande Link, -> check! [D]

                    LINK ------[====]------ Bestaande link
                    REQ  -[==]------------- Request komt voor Link, geen overlap = geen probleem

                    LINK ------[====]------ Bestaande link
                    REQ  -------------[==]- Request komt na Link, geen overlap = geen probleem
                    */
                    //if (
                    //    (l.STARTDATE < req.ValidFrom && l.ENDDATE > req.ValidFrom) || // [A]
                    //    (req.ValidUntil > l.STARTDATE) ||  // [B]
                    //    (req.ValidFrom < l.STARTDATE && req.ValidUntil > l.ENDDATE) ||  // [C]
                    //    (req.ValidFrom > l.STARTDATE && req.ValidUntil < l.ENDDATE)) // [D]

                    //TimeRange rLink = new TimeRange(l.STARTDATE, l.ENDDATE);
                    //TimeRange rReq = new TimeRange(req.ValidFrom, req.ValidUntil);
                    //if (rLink.OverlapsWith(rReq))
                    //{
                    //    // CASE A/B begintijd of eindtijd van de nieuwe aanvraag valt binnen een bestaande link 
                    //    if (l.PROVIDERID != req.ProviderId && provs.Where(s => s.linkType == Provider.LinkType.eXclusive).Count() > 0)
                    //    {
                    //        // overlap, doe niets
                    //        res.Remark = "Exception; eXclusive LinkType, no update";
                    ...
                    //    }
                    //}
                    //else if (req.ValidFrom > l.STARTDATE)
                    //{
                ...
                //else
                //{
                //    // geen overlap nu ondervangen in de query
                //    CreateLink(req.VehicleId, req.CountryCode, req.ProviderId, null, req.ValidFrom, req.ValidUntil, null, req.AreaId, req.VehicleIdType, null);
                //    res.Remark = "Information; new link";
                //    res.RemarkId = "138";
                //}
                ...
                {
                    if (l.ENDDATE > req.ValidFrom)
                    {
                        //update enddate
                        UpdateLink(req.CountryCode, null, l.STARTDATE, req.ValidFrom, null, l.ID, req.VehicleIdType, null);

                        res.Remark = "Information; link overruled, message send to provider: " + l.PROVIDERID;
                        res.RemarkId = "137";
                    }
                    else
                    {
                ...
                else if (l.PROVIDERID != req.ProviderId && provs.Where(s => s.linkType == Provider.LinkType.eXclusive).Count() > 0)
                {
                    if (l.ENDDATE > req.ValidFrom)
                    {
                        // overlap, doe niets
            ...
            */
                //else
                //{
                //    Link lnk = GetLink(req.ProviderId, req.VehicleId, null, null, null, req.AreaId, false, req.VehicleIdType);

                //    if (lnk != null)
                //    {
                //        UpdateLink(req.CountryCode, null, req.ValidFrom, req.ValidUntil, null, lnk.ID, req.VehicleIdType, null);
                //        res.Remark = "[Updated]";
                //    }
                //    else
                //    {
