    ...
    public class Functions
    {
        ...
        public static RDWRight RDWEnrollRight(string providerID, string areaManagerId, string areaId, string usageId, string vehicleId, DateTime startDateTime, DateTime? endDateTime, string countryCode, Decimal? amount, Decimal? vat, string reference)
        {
            ...
            else
            {
                ...
                {
                    Database.Log("xxx; Exception: " + ex.Message);
                }
        ...
        internal static RDW.RegistrationClient RDWClient(X509Certificate2 cert)
        {
            ...
            {
                Database.Log("CLNT; EX: " + ex.Message + "; STACK: " + ex.StackTrace);
            }
        ...
        internal static X509Certificate2 GetCertificate(string provider, Boolean nprRegistration = true)
        {
            ...
            Database.Log(String.Format("GetCertificate provider {0} NPRREG {1}", provider, nprRegistration));

            DataTable dt = Database.ExecuteQuery(com);
            if (dt != null && dt.Rows.Count > 0)
            {
                DataRow dr = dt.Rows[0];
                Database.Log(String.Format("GetCertificate {0} provider {1} NPRREG {2}", dr["FILENAME"], provider, nprRegistration));
